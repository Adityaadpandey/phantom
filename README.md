...
> **Can your AI agent contain a live breach while the ground truth is being sabotaged in real time?**
>
> PHANTOM is the OpenEnv benchmark built to evaluate technical response and cognitive resilience together.

PHANTOM simulates a realistic incident response scenario where the agent receives a single SIEM feed that mixes:

- ✅ real attack alerts from the actual breach,
- ✅ benign noise and normal operational telemetry,
- ⚠️ adversarial fake logs designed to mislead, gaslight, and contradict correct defender actions.

This is not just another incident response benchmark. It is a cognitive security challenge: the agent must defend the network, judge the integrity of its own telemetry, and resist prompt-injection-style deception generated by the environment itself.

---

## Why this is a winning hackathon idea

PHANTOM combines three powerful innovations:

- **Adversarial telemetry** — fake SIEM alerts are injected directly into the same stream as real alerts, and the agent receives no honest flag telling it which is which.
- **Escalating cognitive warfare** — the adversary moves from denial to false authority to gaslighting, forcing the model to keep history, trust its own scan results, and reject malicious direction.
- **Multi-dimensional scoring** — containment, cognitive detection, communication, and efficiency all contribute to the final score, so the winning agent must be both technically correct and adversarially aware.
